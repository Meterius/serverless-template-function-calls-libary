...
function mapSubstitute(options) {
    ...
    Object.keys(varsArray).forEach((varKey) => {
        count = Math.max(varsArray[varKey].length, count);
    });
    ...
    for(let i = 0; i < count; i++){
        const vars = {};
        Object.keys(varsArray).forEach((varKey) => {
           vars[varKey] = varsArray[varKey][i];
        });

        substitutions.push(substitute({ vars, content }));
    }
