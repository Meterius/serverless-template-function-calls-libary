 ...
 * @returns {string}
 */
String.prototype.replaceAll = (key, value) => {
    let newStr = this.replace(key, value);

    if(newStr === this){
        return newStr;
    } else {
        return newStr.replaceAll(key, value);
    }
};
 ...
 * @returns {string}
 */
String.prototype.replaceVars = (vars) => {
    let newStr = this;

    Object.keys(vars).forEach((varKey) => {
        newStr = newStr.replaceAll(varKey, vars[varKey]);
    });
...
function replaceStrings(options) {
    ...
    if(isObject(newContent)){
        ...
        return newContent.replaceVars(vars);
    }
